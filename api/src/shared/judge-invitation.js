...
async function buildJudgeInvitationEmail(invitation, context) {
    try {
        ...
        // Check if event has judge invitation email enabled
        if (!event.sendJudgeInvitationEmail) {
            context.log(`Event ${invitation.eventId} does not have judge invitation email enabled, using basic send`);
        ...
        if (!template) {
            context.warn('Judge invitation template not found in system-email-config.json');
        ...
        context.log(`[DEBUG judge-invite] acceptUrl: '${acceptUrl}', invitation.id: '${invitation.id}'`);
        ...
        const templatePath = path.join(__dirname, '../../data/email-templates/invitation-judge.html');
        ...
        return {
            success: false,
            reason: 'Error building judge invitation email',
            error: error.message
        };
    }
}
